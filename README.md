...
研究迭代记录：币市 8 个时序管线 walk-forward 全部证伪（`docs/results/03-07`，含体制闸门、LLM 假设批次、资金费率反转）；美股四因子初检全拒（`09`，含点时股池与多重检验校正）；A 股四因子初检出现**全项目首个未被拒信号**——沪深 300 动量 INCONCLUSIVE（分层完美单调、一致率 71%、净 +1.33%/月，36 月样本不足以过校正显著性，复检需 ≥10 年样本 + 点时成分，`10`）。所有检验按预登记协议执行（`docs/results/factor-registry.md`），13 个检验对象、12 拒 1 存疑——这套流水线的产出就是诚实本身。
