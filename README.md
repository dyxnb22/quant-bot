...
研究迭代记录：体制闸门（EMA200/EMA100 两版）、均值回归、LLM 复盘提出的紧止损/时间熔断/币对差异化——**七个管线的 walk-forward 全部证伪**（`docs/results/04`、`06`）。部署决策均按预登记标准执行，两轮拒绝了"听起来合理"的改动。累计结论："1h K 线 + 经典技术指标"信息集的天花板已被系统性画出；后续要么换信息源（资金费率/订单簿，阶段二），要么把本仓库当作方法论训练完成的工作台。
